...
/** EditUserInfo ctor */
  ...
  submitForm() {
    ...
    let _obj = {
      ...
      userEnabled: ctls['userEnabled'].value,
      userFailedLoginCount: ctls['userFailedLoginCount'].value
    }
    ...
    this.http.post('/User/EditUserInfo', data, httpOptions).subscribe(
      response => {
        ...
        if (message1 == '000') {
          this.dialogService.sendMessage('000' + 'edituserinfo', _obj.userName);
          this.infoservice.sendConfirmCommand(this.user, this.index);
        }
        if (message1 == '002') {
