...
export class AllApplicationListComponent implements OnDestroy {
/** approve-leave-application ctor */
  ...
  public disableLeaveApplication(index: number): void {
    let data = JSON.stringify(this.apps[index]);

    this.http.post(apiLink + '/LeaveManagement/DisableLeaveApplication', data, res.moduleHttpOptions).subscribe(
      response => {
        let result = JSON.parse(JSON.stringify(response));
        let message = result['status'] as string;
        if (message == '000') {
          this.apps[index].finalStatus = false;
        }
      },
      error => {
  ...
  public getData(pageSize: number, requestPage: number): void {
    ...
    this.http.post(apiLink + '/LeaveManagement/GetFullLeaveApplicationlist', data, res.moduleHttpOptions).subscribe(
      response => {
        let result = JSON.parse(JSON.stringify(response));
  ...
  public downloadCsv(): void {
    ...
    let data = JSON.stringify(x);
    this.http.post(apiLink + '/LeaveManagement/FullLeaveApplicationListDownload', data, res.moduleHttpOptions).subscribe(
      response => {
        //let result = JSON.parse(JSON.stringify(response));
        //this.collectionSize = result['collectionSize'];
        //this.activePage = result['activePage'];
